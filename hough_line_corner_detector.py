...
class HoughLineCornerDetector:
    ...
    def _get_hough_lines(self):
        ...
        outer = np.array([[[0.0,0.0]],[[0.0,0.5 * math.pi]],[[height,0.5 * math.pi]],[[width, 0.0]]])


        if lines is not None:
            lines = np.concatenate((lines, outer))
        else:
            lines = outer


        
        if self.output_process and lines.size > 0: self._draw_hough_lines(lines)
